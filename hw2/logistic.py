...
def load_data(train_data_path, train_label_path, test_data_path):
	# read X_train file and extract its values and store in an numpy array
	X_train = pd.read_csv(train_data_path, sep=',', header=0)
	X_train = np.array(X_train.values)

	# read Y_train file and extract its values and store in an numpy array
	Y_train = pd.read_csv(train_label_path, sep=',', header=0)
	Y_train = np.array(Y_train.values)

	# read X_test file and extract its values and store in an numpy array
...
def sigmoid(z):
    res = 1 / (1.0 + np.exp(-z))
		#return res within 1e-8 and 1-(1e-8) using clip
...
def train(X_all, Y_all, save_dir):
	...
	# Start training
	total_loss = 0.0
	sigma_w, sigma_b = np.zeros((106,)), np.zeros((1,))
	...
	for epoch in range(1, epoch_num):
		...
		for idx in range(step_num):
			...
			#w = w - (l_rate/np.sqrt(sigma_w/it)) * w_grad
			#b = b - (l_rate/np.sqrt(sigma_b/it)) * b_grad
			w = w - l_rate * (w_grad+0.2*w) / np.sqrt(sigma_w)
			b = b - l_rate * b_grad / np.sqrt(sigma_b)
			#it += 1
...
if __name__ == '__main__':
    ...
    group.add_argument('--load', action='store_true',default=False,
                        dest='load', help='Input --load to load')
    ...
    parser.add_argument('--output_dir', type=str,
                        default='logistic_output/log_prediction.csv', dest='output_dir',
                        help='Path to save the model parameters')
